import $ from "jquery";
// Modified version of ublaboo-datagrid.
 ...
 * @param {Naja} naja
 */
export const createDatagrid = (naja) => {
  var najaEventParams = function (params) {
    return params.detail;
  };
  var najaRequest = function (params) {
    return params.detail.request;
  };
  ...
  dataGridRegisterExtension = function (name, extension) {
    ...
    var NewExtension = function NewExtension(name) {
      this.name = name;
      ...
      this.initialize = function (naja) {
        ...
        if (success) {
          naja.addEventListener("success", function (params) {
            var payload = params.detail.payload;
    ...
    naja.registerExtension(new NewExtension(name));
  };
  ...
  dataGridRegisterAjaxCall = function (params) {
    ...
    naja
      .makeRequest(method, params.url, data, {})
      .then(params.success)
      .catch(params.error);
  };
  ...
  $(document)
    .on("change", "select[data-autosubmit-per-page]", function () {
      var button;
    ...
    .on(
      "change",
      "input[data-autosubmit][data-autosubmit-change]",
      function (e) {
        var $this, code;
    ...
    .on("keyup", "input[data-autosubmit]", function (e) {
      ...
      if (
        code !== 13 &&
        ((code >= 9 && code <= 40) || (code >= 112 && code <= 123))
      ) {
        return;
      }
      clearTimeout(window.datagrid_autosubmit_timer);
    ...
    .on("keydown", ".datagrid-inline-edit input", function (e) {
      ...
      if (code === 13) {
        ...
        return $(this)
          .closest("tr")
          .find('.col-action-inline-edit [name="inline_edit[submit]"]')
          .click();
      }
    });
      ...
      for (i = 0, len = ref.length; i < len; i++) {
        ...
        if ($(el).is(".col-checkbox") && last_checkbox && e.shiftKey) {
          ...
          if (current_checkbox_row.index() > last_checkbox_row.index()) {
            rows = checkboxes_rows.slice(
              last_checkbox_row.index(),
              current_checkbox_row.index()
            );
          } else if (current_checkbox_row.index() < last_checkbox_row.index()) {
            rows = checkboxes_rows.slice(
              current_checkbox_row.index() + 1,
              last_checkbox_row.index()
            );
          }
          if (!rows) {
  ...
  document.addEventListener("change", function (e) {
    ...
    if (grid) {
      checked_inputs = document.querySelectorAll(
        "input[data-check-all-" + grid + "]:checked"
      );
      select = document.querySelector(
        ".datagrid-" + grid + ' select[name="group_action[group_action]"]'
      );
      buttons = document.querySelectorAll(
        ".datagrid-" + grid + ' .row-group-actions *[type="submit"]'
      );
      counter = document.querySelector(
        ".datagrid-" + grid + " .datagrid-selected-rows-count"
      );
    ...
    if (grid) {
      inputs = document.querySelectorAll(
        "input[type=checkbox][data-check-all-" + grid + "]"
      );
      results = [];
  ...
  datagridSortable = function () {
    ...
    return $(".datagrid [data-sortable]").sortable({
      ...
      update: function (event, ui) {
        ...
        data = {};
        component_prefix = row
          .closest(".datagrid")
          .find("tbody")
          .attr("data-sortable-parent-path");
    ...
    datagridSortableTree = function () {
      ...
      return $(".datagrid-tree-item-children").sortable({
        ...
        stop: function (event, ui) {
          return $(".toggle-tree-to-delete").removeClass(
            "toggle-tree-to-delete"
          );
        },
        start: function (event, ui) {
          ...
          if (parent.length) {
            if (parent.find(".datagrid-tree-item").length === 2) {
              return parent
                .find("[data-toggle-tree]")
                .addClass("toggle-tree-to-delete");
            }
          }
        },
      });
  ...
  dataGridRegisterExtension("datagrid.happy", {
    success: function () {
      ...
      for (i = 0, len = grids.length; i < len; i++) {
        ...
        checked_rows = document.querySelectorAll(
          class_selector + " " + "input[data-check]:checked"
        );
        if (
          checked_rows.length === 1 &&
          checked_rows[0].getAttribute("name") === "toggle-all"
        ) {
          input = document.querySelector(
            class_selector + " input[name=toggle-all]"
          );
          if (input) {
            input.checked = false;
            ie = window.navigator.userAgent.indexOf("MSIE ");
    ...
    success: function (payload) {
      ...
      if (payload._datagrid_url) {
        if (window.history.replaceState) {
          ...
          query = window
            .datagridSerializeUrl(payload.state)
            .replace(/&+$/gm, "");
          if (query) {
  ...
  dataGridRegisterExtension("datargid.item_detail", {
    before: function (xhr, settings) {
      ...
      if (settings.nette && settings.nette.el.attr("data-toggle-detail")) {
        ...
        grid_fullname = settings.nette.el.attr(
        ...
        if (row_detail.hasClass("loaded")) {
          ...
          if (row_detail.hasClass("toggled")) {
            row_detail.find(".item-detail-content").slideToggle(
              "fast",
              (function (_this) {
                return function () {
  ...
  dataGridRegisterExtension("datagrid.tree", {
    before: function (xhr, settings) {
      ...
      if (settings.nette && settings.nette.el.attr("data-toggle-tree")) {
        ...
        children_block = settings.nette.el
          .closest(".datagrid-tree-item")
          .find(".datagrid-tree-item-children")
          .first();
    ...
    success: function (payload) {
      ...
      if (payload._datagrid_tree) {
        ...
        children_block = $('.datagrid-tree-item[data-id="' + id + '"]')
          .find(".datagrid-tree-item-children")
          .first();
  ...
  $(document).on("click", "[data-datagrid-editable-url]", function (event) {
    ...
    if (!cell.hasClass("editing")) {
      ...
      if (cell.data("datagrid-editable-type") === "textarea") {
        ...
        cell_padding = parseInt(
          cell.css("padding").replace(/[^-\d\.]/g, ""),
          10
        );
  ...
  dataGridRegisterExtension("datagrid.after_inline_edit", {
    success: function (payload) {
      ...
      if (payload._datagrid_inline_edited) {
        grid
          .find("tr[data-id=" + payload._datagrid_inline_edited + "] > td")
          .addClass("edited");
  ...
  $(document).on("mouseup", "[data-datagrid-cancel-inline-add]", function (e) {
    ...
    if (code === 1) {
      e.stopPropagation();
      e.preventDefault();
      return $(".datagrid-row-inline-add").addClass(
  ...
  dataGridRegisterExtension("datagrid-toggle-inline-add", {
    success: function (payload) {
      ...
      if (payload._datagrid_inline_adding) {
        ...
        row
          .find("input:not([readonly]),textarea:not([readonly])")
          .first()
          .focus();
      }
    },
    ...
    success: function (payload) {
      ...
      if (payload.non_empty_filters && payload.non_empty_filters.length) {
        ...
        for (i = 0, len = ref.length; i < len; i++) {
          key = ref[i];
          grid
            .find("[data-datagrid-reset-filter-by-column=" + key + "]")
            .removeClass("hidden");
        }
        href = grid.find(".reset-filter").attr("href");
        return grid
          .find("[data-datagrid-reset-filter-by-column]")
          .each(function () {
            var new_href;
            key = $(this).attr("data-datagrid-reset-filter-by-column");
            new_href = href.replace(
              "do=" + payload._datagrid_name + "-resetFilter",
              "do=" + payload._datagrid_name + "-resetColumnFilter"
            );
